...
This project uses a staged, reconcile-only recovery model.

Data is never imported directly into the live database.

Instead:

1. import into a temporary database
2. analyze schema differences
3. assign strategies per table
4. execute only safe operations
5. verify outcomes
6. report results
...
If a table cannot be handled safely, it is not forced.
